...
사용자가 `이슈커밋푸시 진행해` (또는 동등한 표현)를 명시하면, 해당 작업에 한해 이슈 생성 → 검증 → 커밋 → 푸시를 순서대로 실행한다.

상세 순서, 중단 조건, 권한 기준은 **[docs/WORKFLOW.md — 이슈커밋푸시 자동 진행 규칙](docs/WORKFLOW.md)** 을 따른다.
...
- 외부 사이트 콘텐츠 무단 복사
- **사용자 명시적 지시 없이 commit 실행** (`이슈커밋푸시 진행해` 명시 시는 예외)
- **사용자 명시적 지시 없이 push 실행** (`이슈커밋푸시 진행해` 명시 시는 예외)
- 작업 범위 밖의 파일 수정
